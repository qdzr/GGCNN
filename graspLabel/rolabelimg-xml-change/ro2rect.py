...
for xml_file in os.listdir(xml_path):
    ...
    for ob in root.iter('object'):
        ...
        if ob.find('bndbox') is not None:
            ...
            line = rect['path'] + "\t" + rect['xmin'] + "\t" + rect['ymin'] + "\t" + rect['xmax'] + "\t" + rect[
                'ymax'] + '\n'

            # f1 = open('./IMG_6550_all.txt', 'a')
            # f1.write(line)

            line2 = rect['xmin'] + "," + rect['ymin'] + "," + rect['xmax'] + "," + rect['ymin'] + ',' + rect[
                'xmax'] + ',' + rect['ymax'] + ',' + rect['xmin'] + ',' + rect['ymax'] + ',' + 'textline' + '\n'
        ...
        if ob.find('robndbox') is not None:
            ...
            line = rect['path'] + "\t" + rect['cx'] + "\t" + rect['cy'] + '\t' + rect['angle'] + "\t" + rect[
                'w'] + "\t" + rect['h'] + '\n'
            ...
            ro = addRotatedShape(float(rect['cx']), float(rect['cy']), float(rect['w']), float(rect['h']),
                                 float(rect['angle']))
            # line2 = str(ro[0])+','+ str(ro[1])+','+ str(ro[2])+','+ str(ro[3])+','+ str(ro[4])+','+ str(ro[5])+','+ str(ro[6])+','+ str(ro[7])+','+'textline'+'\n'
            line2 = str(int(ro[0])) + ',' + str(int(ro[1])) + ',' + str(int(ro[2])) + ',' + str(int(ro[3])) + ',' + str(
                int(ro[4])) + ',' + str(int(ro[5])) + ',' + str(int(ro[6])) + ',' + str(
                int(ro[7])) + ',' + 'textline' + '\n'
            f2 = open(save_xml_path + xml_file[:-4] + '.txt', 'a')
            f2.write(line2)
